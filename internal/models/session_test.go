...
func TestZeroTime_IsInPast(t *testing.T) {
	z := ZeroTime()
	if !time.Now().After(z) {
		t.Fatal("ZeroTime should be in the past")
	}
}

func TestFutureTime_IsInFuture(t *testing.T) {
	f := FutureTime()
	if !f.After(time.Now()) {
		t.Fatal("FutureTime should be in the future")
	}
}
